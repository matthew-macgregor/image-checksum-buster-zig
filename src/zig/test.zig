...
fn create_minimal_jpeg(path: []const u8) !void {
    try create_any_file(path, &minimal_jpeg);
}

fn create_any_file(path: []const u8, buffer: []const u8) !void {
    const file = try std.fs.cwd().createFile(path, .{});
    defer file.close();
    try file.writeAll(buffer);
}

fn slurp_file(alloc: std.mem.Allocator, path: []const u8) ![]u8 {
...
fn md5_file(alloc: std.mem.Allocator, path: []const u8, hash_out: *[16]u8) !void {
    const buffer = try slurp_file(alloc, path);
...
test "icbust_file" {
    ...
    try setup();
    try img.icbust_file(filen_in, filen_out, debug);
}
...
test "icbust_file with bad file" {
    ...
    try setup();
    try create_any_file(filen_in, &[_]u8{ 0x35, 0x36 });
    img.icbust_file(filen_in, filen_out, debug) catch |err| {
        try std.testing.expect(err == img.StbImageError.IOInputError);
    };
}
...
test "md5 checksums" {
    ...
    try setup();
    try img.icbust_file(in, out, debug);
    const alloc = std.testing.allocator;
    var hash1: [16]u8 = undefined;
    try md5_file(alloc, in, &hash1);
    var hash2: [16]u8 = undefined;
    try md5_file(alloc, out, &hash2);
